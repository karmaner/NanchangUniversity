...
class scheduler:
    ...
    def load(self, program_description):
        ...
        if len(tmp) != 2:
            print '错误描述 (%s): 必须是数字 <x:y>' % program_description
            print '  其中 X 是指令数, Y 是CPU运行指令或发出IO的机会'
            exit(1)
...
parser = OptionParser()
parser.add_option('-s', '--seed', default=0, help='随机种子'.decode('utf-8'), action='store', type='int', dest='seed')
parser.add_option('-l', '--processlist', default='',
                  help='要运行的进程的逗号分隔列表, 形式： X1:Y1,X2:Y2,... \t\t\t\t其中 X 是运行的进程的指令数，\t\t\t\t\tY 是CPU运行指令或发出IO的机会（从 0 到 100 ）'.decode('utf-8'),
                  action='store', type='string', dest='process_list')
parser.add_option('-L', '--iolength', default=5, help='IO 需要多长时间(指从发出IO到完成，默认是5)'.decode('utf-8'), action='store', type='int', dest='io_length')
parser.add_option('-S', '--switch', default='SWITCH_ON_IO',
                  help='何时在进程之间切换: SWITCH_ON_IO, SWITCH_ON_END'.decode('utf-8'),
                  action='store', type='string', dest='process_switch_behavior')
parser.add_option('-I', '--iodone', default='IO_RUN_LATER',
                  help='IO 结束时的行为类型: IO_RUN_LATER, IO_RUN_IMMEDIATE'.decode('utf-8'),
...
parser.add_option('-p', '--printstats', help='最后打印统计信息；随 -c 参数生效(否则不打印统计信息)'.decode('utf-8'), action='store_true', default=False, dest='print_stats')
...
if options.solve == False:
    ...
    if options.process_switch_behavior == SCHED_SWITCH_ON_IO:
        print '当前进程 已完成 或 发出IO'
    else:
        print '当前进程 已完成'
    print '  在 IO 完成之后，发出 IO 的进程将',
